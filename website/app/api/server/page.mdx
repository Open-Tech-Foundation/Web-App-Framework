---
title: Server
description: The @opentf/web/server surface — request middleware, API routes, createFetchHandler, and runtime adapters.
---
...
The `@opentf/web/server` entry powers file-based [API routes](/docs/routing/api-routes)
and [request middleware](/docs/routing/middleware). Most apps never call these directly —
...
| `apiRoutes` | API dispatch only — pair with `middleware` in `createFetchHandler` |
| `apiHandler` | Standalone composed handler (API routes + API-folder middleware) |
