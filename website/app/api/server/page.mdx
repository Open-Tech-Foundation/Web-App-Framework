---
title: Server
description: The @opentf/web/server surface — middleware, cookies, API routes, createFetchHandler, and runtime adapters.
---
...
See [Fetch handler](/docs/deployment/fetch-handler) for a full edge deployment walkthrough.
...
Standards-based `Cookie` / `Set-Cookie` helpers for [middleware](/docs/routing/middleware),
[API routes](/docs/routing/api-routes), and [loaders](/docs/data-fetching). Guide:
[Cookies](/docs/routing/cookies). Shipped in `@opentf/web@0.16.0`.
...
<Callout type="info" title="Immutable response headers">
  A response returned from `fetch` or `next()` may have immutable headers. Wrap before
  calling `setCookie`: `const wrapped = new Response(res.body, res)`.
</Callout>
