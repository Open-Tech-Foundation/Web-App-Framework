...
`otfw dev` watches `app/` and, on a supported file change, drops cached route chunks and
signals the browser over `/__hmr` to **`location.reload()`** — a full tab refresh, not
in-place hot module replacement.
