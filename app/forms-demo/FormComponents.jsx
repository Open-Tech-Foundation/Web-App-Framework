...
export function FormField(props) {
  return (
    <div className="flex flex-col gap-1 mb-4">
      <label className="text-sm font-semibold text-slate-300">{props.label}</label>
      <input 
        {...props} 
        className={`px-3 py-2 rounded-lg border transition-all outline-none bg-slate-900 text-white ${
          props.error ? 'border-red-500/50 bg-red-500/5' : 'border-slate-700 focus:border-blue-500'
        }`}
      />
      {props.error && <span className="text-xs text-red-400 font-medium">{props.error}</span>}
    </div>
  );
}
...
export function CustomToggle(props) {
  ...
  return (
    <div className="flex items-center gap-3 mb-4 cursor-pointer" onclick={toggle}>
      <div className={`w-10 h-5 rounded-full transition-colors relative ${props.value ? 'bg-blue-600' : 'bg-slate-700'}`}>
        <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${props.value ? 'left-6' : 'left-1'}`} />
      </div>
...
export function BasicForm() {
  const form = createForm({
    ...
    validate: (v) => {
      const errs = {};
      if (!v.username) errs.username = "Username is required";
  ...
  return (
    <section className="bg-slate-800/40 backdrop-blur-xl p-8 rounded-2xl border border-slate-700/50 shadow-2xl">
      ...
      <form onsubmit={form.handleSubmit((v) => console.log('Basic Submit:', v))}>
        <FormField label="Username" {...form.register('username')} error={form.errors.value.username} />
        <FormField label="Email" {...form.register('email')} error={form.errors.value.email} />
...
/**
 * Complex Form Example with Nested Objects and Dynamic Arrays
 */
export function ComplexForm() {
  const form = createForm({
    initialValues: {
      ...
      skills: ["JavaScript", "MWAF"] 
    },
    validate: (v) => {
      const errs = {};
      if (!v.profile.firstName) errs["profile.firstName"] = "First name is required";
      return errs;
    }
  });
  ...
  return (
    <section className="bg-slate-800/40 backdrop-blur-xl p-8 rounded-2xl border border-slate-700/50 shadow-2xl">
      ...
      <form onsubmit={form.handleSubmit((v) => console.log('Complex Submit:', v))}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField label="First Name" {...form.register('profile.firstName')} error={form.errors.value["profile.firstName"]} />
